...
export default function WaiverBanner({ destination }: { destination: string }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-brand-800 via-brand-700 to-brand-600 p-6 text-white shadow-2xl shadow-brand-600/30 sm:p-7">
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 font-display text-2xl font-extrabold">
              <span className="mr-2 text-brand-200 line-through decoration-hot decoration-2">€300</span>
        ...
        <div className="flex flex-wrap items-center gap-5">
          <div className="flex gap-2">
            {cells.map((c) => (
              <div key={c.l} className="min-w-[3.2rem] rounded-xl bg-brand-950/60 px-2 py-2 text-center">
          ...
          <div className="text-right">
            <p className="mb-1.5 text-[11px] font-bold text-brand-200">Offer ends soon. Apply now!</p>
            <Link
              href={`/apply/?destination=${destination}`}
              className="inline-block rounded-full bg-white px-6 py-3 font-bold text-brand-800 shadow-lg transition-transform duration-300 hover:-translate-y-0.5"
            >
              Apply Now →
            </Link>
          </div>
