      ...
      <div className="relative mx-auto max-w-7xl px-5 lg:px-8">
            ...
            <Reveal key={t.id} delay={i * 100}>
              ...
              >
                ...
                <span className="absolute inset-0 grid place-items-center opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                  <span className="grid h-16 w-16 place-items-center rounded-full bg-white/90 text-ink shadow-xl transition-transform duration-300 group-hover:scale-110">
        ...
        <div className="mt-8 flex justify-center gap-2" aria-hidden="true">
          {testimonials.map((t, i) => (
            <span key={t.id} className={`h-1.5 rounded-full transition-all ${i === 0 ? "w-6 bg-white" : "w-1.5 bg-white/25"}`} />
          ))}
        </div>
      </div>
        ...
        >
          ...
          >
            <button
              ...
              aria-label="Close"
              onClick={() => setOpenId(null)}
              className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-surface text-ink transition-colors hover:bg-line"
            >
              <X className="h-4 w-4" />
            </button>
            <Quote className="h-8 w-8 text-white/60" aria-hidden="true" />
            ...
            <p className="text-sm text-study">Now in {active.destination}</p>
